...
const pool = new Pool({
  ...
  min: 5,  // Keep 5 connections always ready (prevents "load failed")
  idleTimeoutMillis: 30000,
  connectionTimeoutMillis: 10000,
  keepAlive: true,
  keepAliveInitialDelayMillis: 10000
...
app.get('/api/health', (req, res) => {
  res.json({ 
    ...
    features: ['transactions', 'connection-pool', 'coalesce-nulls']
  });
});
...
initializeDatabase()
  .then(() => {
    app.listen(PORT, '0.0.0.0', () => {
      console.log(`✅ RNPathfinders API v3.0.5 (STABLE - Load Failed Fixed) running on port ${PORT}`);
    console.log(`🔗 Connection Pool: max=20, min=5 (always ready)`);
