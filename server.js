...
app.post('/api/resources', verifyToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO resources (title, url, category, type) VALUES ($1, $2, $3, $4) RETURNING *',
      [title, url, category, type]
    );

    res.json({ message: 'Resource added successfully', resource: result.rows[0] });
  } catch (error) {
...
app.post('/api/admin/generate-codes', verifyToken, async (req, res) => {
  try {
    if (!req.user.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    }
